  ...
  init: (controller) => {
    controller.hears('<!channel|<!group|<!here|<!everyone', 'ambient', (bot, message) => {
      ...
      bot.startPrivateConversation({user: message.user}, function(err, privateConvo) {
        if (err) return;
        privateConvo.say(`Howdy! I noticed you used \`${match}\` in the channel. That can wake people up, disturb them, or can be a bit annoying.
Please refrain from \`@channel\`, \`@here\`, \`@everyone\`, or \`@group\`. Thanks!`);
  ...
  help: {
    command: 'hush',
    text: `I will listen in on any channel you invite me to, if anyone uses \`@channel\`, \`@here\`, \`@everyone\`, or \`@group\`
